...
!
!
MODULE B2MOD_PAR_OPT_DIFFV_DIFFV
  USE B2MOD_TYPES
...
& flag_hci, flag_hce, flag_vla, flag_vsa, flag_sig, flag_alf
  USE B2MOD_INPUT_PROFILE_DIFFV_DIFFV, ONLY : ndata
...
!parameters for prior
  ...
  REAL(kind=r8), SAVE :: sigmad0(nbdirsmax0, nsigmx)=0.D0
  ...
  REAL(kind=r8), SAVE :: meand0(nbdirsmax0, nsigmx)=0.D0
...
& nsigmx, 2)
  ...
  REAL(kind=r8), SAVE :: corr_lengthd0(nbdirsmax0, nncf)=0.D0
...
CONTAINS
!  Differentiation of read_b2mod_par_opt_dv as a context to call tangent code (with options multiDirectional context noISIZE r8):
!   Plus diff mem management of: b2data:in-out b2dataoncf:in-out
!                b2datad:in-out b2dataoncfd:in-out b2voloncf:in-out
!                b2voloncfd:in-out
!  Differentiation of read_b2mod_par_opt as a context to call tangent code (with options multiDirectional context noISIZE r8):
!   Plus diff mem management of: b2data:in-out b2dataoncf:in-out
!                b2voloncf:in-out m.cffcor:in-out
!
  SUBROUTINE READ_B2MOD_PAR_OPT_DV_DV(ns, m, md0, sw, nbdirs, nbdirs0)
    USE B2MOD_TYPES
...
! now allocate vector that will be used to interpolate SOLPS onto data grid
    IF (ANY(cfread)) THEN
      ...
      ALLOCATE(b2voloncfdd(nbdirsmax0, nbdirsmax, ncf, numdata))
      b2voloncfdd = 0.D0
      ALLOCATE(b2voloncfd(nbdirsmax, ncf, numdata))
      b2voloncfd = 0.d0
      ALLOCATE(b2voloncfd0(nbdirsmax0, ncf, numdata))
      b2voloncfd0 = 0.D0
      ALLOCATE(b2voloncf(ncf, numdata))
!stores interpolated B2.5 volumes onto CF radial points
      ALLOCATE(b2dataoncfdd(nbdirsmax0, nbdirsmax, numdata))
      b2dataoncfdd = 0.D0
      ALLOCATE(b2dataoncfd(nbdirsmax, numdata))
      b2dataoncfd = 0.d0
      ALLOCATE(b2dataoncfd0(nbdirsmax0, numdata))
      b2dataoncfd0 = 0.D0
...
!store here radial distance coordinate of SOLPS data
      ALLOCATE(b2datadd(nbdirsmax0, nbdirsmax, numdata))
      b2datadd = 0.D0
      ALLOCATE(b2datad(nbdirsmax, numdata))
      b2datad = 0.d0
      ALLOCATE(b2datad0(nbdirsmax0, numdata))
      b2datad0 = 0.D0
...
!
    RETURN
  END SUBROUTINE READ_B2MOD_PAR_OPT_DV_DV
...
!  Differentiation of dealloc_b2mod_par_opt_dv as a context to call tangent code (with options multiDirectional context noISIZE r
!8):
!   Plus diff mem management of: b2data:out b2dataoncf:out b2datad:out
!                b2dataoncfd:out b2voloncf:out b2voloncfd:out
...
!   Plus diff mem management of: b2data:out b2dataoncf:out b2voloncf:out
!
  SUBROUTINE DEALLOC_B2MOD_PAR_OPT_DV_DV(nbdirs, nbdirs0)
!  Hint: nbdirsmax should be the maximum number of differentiation directions
    USE B2MOD_DIFFSIZES
!  Hint: nbdirsmax0 should be the maximum number of differentiation directions
    IMPLICIT NONE
    ...
    IF (ALLOCATED(b2voloncf)) THEN
      IF (ALLOCATED(b2voloncfd)) THEN
        IF (ALLOCATED(b2voloncfdd)) THEN
          DEALLOCATE(b2voloncfdd)
        END IF
        DEALLOCATE(b2voloncfd)
      END IF
      IF (ALLOCATED(b2voloncfd0)) THEN
        DEALLOCATE(b2voloncfd0)
      END IF
      DEALLOCATE(b2voloncf)
    END IF
    IF (ALLOCATED(b2data)) THEN
      IF (ALLOCATED(b2datad)) THEN
        IF (ALLOCATED(b2datadd)) THEN
          DEALLOCATE(b2datadd)
        END IF
        DEALLOCATE(b2datad)
      END IF
      IF (ALLOCATED(b2datad0)) THEN
        DEALLOCATE(b2datad0)
      END IF
      DEALLOCATE(b2data)
    END IF
    IF (ALLOCATED(b2dataoncf)) THEN
      IF (ALLOCATED(b2dataoncfd)) THEN
        IF (ALLOCATED(b2dataoncfdd)) THEN
          DEALLOCATE(b2dataoncfdd)
        END IF
        DEALLOCATE(b2dataoncfd)
      END IF
      IF (ALLOCATED(b2dataoncfd0)) THEN
        DEALLOCATE(b2dataoncfd0)
      END IF
...
END MODULE B2MOD_PAR_OPT_DIFFV_DIFFV
