...
!.specification
!
SUBROUTINE B2STBR_B(ncv, nfc, nvx, ns, nxtl, nxtr, nscx, nscxmax, iscx, &
& ismain, dtim, switch, geo, geob, mpg, mpgb, st, stb, st_ext, st_extb, &
& main_call)
  USE B2MOD_TYPES
...
& , int1lb, int4r, int4l, int4lb, fna_mol
  ...
  USE B2MOD_AD_DIFF, ONLY : ntstep_b2wall, old_deposition, old_erosion, &
...
& mpg%nfc, ns-1)
  REAL(kind=r8) :: fluid_frac_hybb(nfc), kin_frac_hybb(mpg%nfc)
!   ..common blocks
!-----------------------------------------------------------------------
!.documentation
!
...
&                srw, tchem, tchee, tphys, tphye, thevp, thvpe, trese, &
&                tresn, trfln, trfle, switch%b2stbc_boundary_namelist, &
&                switch%neutral_sources_rescale, switch%&
&                b2stbr_core_sources_rescale, sput_src, switch%&
&                b2stbr_sput_chem_alpha, switch%sput_phys_alpha, switch%&
&                b2stbr_alpha, sput_chem_model, switch%b2stbr_therm_evap&
&                , switch%b2stbr_sput_res, reflection_on, &
&                sputter_energy_on, main_call, switch%b2stbr_output, &
&                switch%b2stbr_sput_frac_flag, switch%&
&                b2stbr_sput_chem_cutoff_alpha, switch%&
&                b2stbr_sput_chem_cutoff_beta, switch%&
&                b2stbr_sput_mixed_alpha, switch%b2stbr_sput_mixed_beta&
&                , new_sputter_namelist, shi0_ff, f_redep, switch%&
&                b2stbr_temperature_at_guard_cell, switch%&
&                b2stbr_potential_at_guard_cell, switch%&
&                recycled_neutrals_contr, fluid_frac_hyb, &
&                fluid_frac_hybb, kin_frac_hyb, kin_frac_hybb, fnn_inc)
  END IF
  arg1 = ncv*4
  arg1 = ncv*4*ns
  arg1 = ncv*2*ns
...
!srv 07.04.09
    CALL IPGETI('b2stbr_ncall_iout', ncall_iout)
!srv 23.03.12
    IF (switch%b2stbr_first_flight .NE. 0) CALL XERRAB(&
&                        'b2stbr -- first flight options not av. for WG'&
&                                               )
    CALL IPGETI('b2stbr_sput_src', sput_src)
    CALL IPGETI('b2stbr_sput_chem_model', sput_chem_model)
    CALL IPGETI('b2stbr_reflection_on', reflection_on)
...
!.specification
!
SUBROUTINE B2STBR_INIT_B(ns, switch, mpg, mpgb)
  USE B2MOD_TYPES
  USE B2MOD_TIME
  USE B2MOD_WALL_DIFF
  USE B2MOD_B2CMPA_DIFF
