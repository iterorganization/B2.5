...
!  Differentiation of b2stbr_init as a context to call adjoint code (with options context noISIZE r8):
!   Plus diff mem management of: mpg.rcfcor:in-out
!
!-----------------------------------------------------------------------
!.specification
!
SUBROUTINE B2STBR_INIT_B(ns, switch, switchb, mpg, mpgb)
  USE B2MOD_TYPES
  USE B2MOD_TIME
  USE B2MOD_WALL
  USE B2MOD_B2CMPA
...
!  Differentiation of b2stbr as a context to call adjoint code (with options context noISIZE r8):
...
!.specification
!
SUBROUTINE B2STBR_B(ncv, nfc, nvx, ns, nscx, nscxmax, iscx, ismain, dtim&
& , switch, switchb, geo, geob, mpg, mpgb, st, stb, st_ext, st_extb, &
& st_avg, main_call)
  USE B2MOD_TYPES
  USE B2MOD_OPENMP
...
!WG_TODO      use b2mod_sputter
  ...
  USE B2US_MAP_DIFF
  USE B2US_PLASMA_DIFF
  USE B2MOD_AD_DIFF, ONLY : ncall_b2stbr, old_deposition, old_erosion, &
...
& , int1lb, int4r, int4l, int4lb, fna_mol
  USE B2MOD_BOUNDARY_NAMELIST_DIFF, ONLY : nbcd, potpar, potparb
  USE B2MOD_AD_DIFF, ONLY : ntstep_b2wall, my_out_folder, filename_b2w, &
& ncall_b2stbr_phys
...
&     neutrals_time_switch, INT(tim/neutrals_time_mod), neutrals_count
      catch_up = (MOD(tim, neutrals_time_mod) .GE. neutrals_time_switch &
&       .AND. neutrals_time_switch .GT. 0.0_R8) .OR. INT(tim/&
&       neutrals_time_mod) .GT. neutrals_count
    ELSE IF (neutrals_time_switch .GT. 0.0_R8) THEN
      catch_up = tim .GE. neutrals_time_switch
...
&                   neutrals_filename)
    ...
    END IF
    IF (catch_up) THEN
      IF (neutrals_time_mod .GT. 0.0_R8) neutrals_count = INT(tim/&
&         neutrals_time_mod)
      CALL READ_NEUTRALS_NAMELIST_B(ns, mpg, mpgb, switch, .true.)
      new_sputter_namelist = .true.
      CALL XERTST(nstrat .LE. nstraid, &
...
!   ..initialize sources to 0
  arg1 = ncv*2*ns
  CALL SFILL_NODIFF(arg1, 0.0_R8, st%srw%sna0, 1)
  CALL PUSHINTEGER4(arg1)
  arg1 = ncv*4*ns
  CALL SFILL_NODIFF(arg1, 0.0_R8, st%srw%smo0, 1)
  CALL PUSHINTEGER4(arg1)
  arg1 = ncv*2
  CALL SFILL_NODIFF(arg1, 0.0_R8, st%srw%sne0, 1)
  CALL PUSHINTEGER4(arg1)
  arg1 = ncv*4
  CALL SFILL_NODIFF(arg1, 0.0_R8, st%srw%she0, 1)
  CALL PUSHINTEGER4(arg1)
  arg1 = ncv*4
  CALL SFILL_NODIFF(arg1, 0.0_R8, st%srw%shi0, 1)
  CALL PUSHINTEGER4(arg1)
  arg1 = ncv*4
  CALL SFILL_NODIFF(arg1, 0.0_R8, st%srw%shn0, 1)
  CALL PUSHINTEGER4(arg1)
  arg1 = ncv*4
  CALL SFILL_NODIFF(arg1, 0.0_R8, st%srw%sch0, 1)
  CALL PUSHINTEGER4(arg1)
  arg1 = ncv*nscx
...
!.specification
!
SUBROUTINE B2STBR_B0(ncv, nfc, nvx, ns, nscx, nscxmax, iscx, ismain, &
& dtim, switch, switchb, geo, geob, mpg, mpgb, st, stb, st_ext, st_extb&
& , st_avg, main_call)
  USE B2MOD_TYPES
  USE B2MOD_OPENMP
!WG_TODO      use b2mod_wall
...
&   %ne + she0_eir(:, 3)*st%pl%te*st%dv%ne
  wrk0b = 0.D0
  wrk0b = -she_eir_totb
  stb%pl%te = stb%pl%te + (she0_eir(:, 1)+st%dv%ne*she0_eir(:, 3))*wrk0b
  stb%dv%ne = stb%dv%ne + (she0_eir(:, 2)+st%pl%te*she0_eir(:, 3))*wrk0b
  CALL POPCONTROL1B(branch)
  IF (branch .EQ. 0) CALL B2STBR_PHYS_B(ncv, nfc, nvx, ns, nscx, nscxmax&
&                                 , iscx, dtim, switch, switchb, geo, &
&                                 geob, mpg, mpgb, st%pl, stb%pl, st%dv&
&                                 , stb%dv, st%co, st%rt, stb%rt, st_ext&
&                                 , st%srw, stb%srw, tchem, tchee, tphys&
&                                 , tphye, thevp, thvpe, trese, tresn, &
&                                 trfln, trfle, sput_src, &
...
&         )
  arg1 = ncv*4
  CALL SFILL_BWD(arg1, 0.0_R8, dummydiffb3, st%srw%shn0, stb%srw%shn0, 1&
&         )
  arg1 = ncv*4
  CALL SFILL_BWD(arg1, 0.0_R8, dummydiffb2, st%srw%shi0, stb%srw%shi0, 1&
&         )
  arg1 = ncv*4
  CALL SFILL_BWD(arg1, 0.0_R8, dummydiffb1, st%srw%she0, stb%srw%she0, 1&
&         )
  CALL POPINTEGER4(arg1)
  arg1 = ncv*4*ns
  CALL SFILL_BWD(arg1, 0.0_R8, dummydiffb0, st%srw%smo0, stb%srw%smo0, 1&
&         )
  arg1 = ncv*2*ns
...
& int6l, int2r, int2l, int5r, int5l, int1r, int1l, int4r, int4l, fna_mol
  USE B2MOD_BOUNDARY_NAMELIST_DIFF, ONLY : nbcd, potpar
  USE B2MOD_AD_DIFF, ONLY : ntstep_b2wall, my_out_folder, filename_b2w, &
& ncall_b2stbr_phys
