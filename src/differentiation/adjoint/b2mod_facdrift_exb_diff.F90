...
!
!
MODULE B2MOD_FACDRIFT_EXB_DIFF
  USE B2MOD_TYPES
  IMPLICIT NONE
!
...
&         , facvis_start, ' towards ', facvis_target
      ...
      ds(-1) = SQRT((result1-0.5_R8*(crx(fac_ref, -1, 0)+crx(fac_ref, -1&
&       , 1)))**2 + (result2-0.5_R8*(cry(fac_ref, -1, 0)+cry(fac_ref, -1&
&       , 1)))**2)
      DO iy=0,ny
        result1 = CR(fac_ref, iy)
        arg1 = iy - 1
        result2 = CR(fac_ref, arg1)
        result3 = CZ(fac_ref, iy)
        arg2 = iy - 1
        result4 = CZ(fac_ref, arg2)
        ds(iy) = ds(iy-1) + SQRT((result1-result2)**2 + (result3-result4&
&         )**2)
      END DO
...
!
      ...
      IF (facdrift_tanh_b .NE. 0.0_R8) THEN
        DO iy=-1,ny
          DO ix=-1,nx
            IF (on_closed_surface(ix, iy)) facdrift_profile(ix, iy) = &
&               facdrift_profile(ix, iy)*0.5_R8*(1.0_R8+TANH((ds(iy)-&
&               facdrift_tanh_a)/facdrift_tanh_b))
          END DO
        END DO
      END IF
...
&                                          , facexb_tanh_b)
      IF (facexb_tanh_b .NE. 0.0_R8) THEN
        DO iy=-1,ny
          DO ix=-1,nx
            IF (on_closed_surface(ix, iy)) fac_exb_profile(ix, iy) = &
&               fac_exb_profile(ix, iy)*0.5_R8*(1.0_R8+TANH((ds(iy)-&
&               facexb_tanh_a)/facexb_tanh_b))
          END DO
        END DO
      END IF
...
&                                          , facvis_tanh_b)
      IF (facvis_tanh_b .NE. 0.0_R8) THEN
        DO iy=-1,ny
          DO ix=-1,nx
            IF (on_closed_surface(ix, iy)) fac_vis_profile(ix, iy) = &
&               fac_vis_profile(ix, iy)*0.5_R8*(1.0_R8+TANH((ds(iy)-&
...
!
  SUBROUTINE B2NEWS_FACDRIFT_FAC_EXB_UPDATE()
    USE B2MOD_PLASMA_DIFF, ONLY : facdrift, fac_exb, fac_vis
...
END MODULE B2MOD_FACDRIFT_EXB_DIFF
