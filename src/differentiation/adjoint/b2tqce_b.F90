...
!     for adjoint AD to avoid side-effect variables
  USE B2MOD_AD_DIFF, ONLY : ncall_b2tqce, ncall_b2tlnl
  USE B2MOD_SUBSYS
  IMPLICIT NONE
!
...
!
!srv 15.06.18
  CALL PUSHREAL8ARRAY(sig, r8*ncv*2/8)
  sig(:, :) = sig(:, :)*switch%prl_cur
!srv 15.06.18
  CALL PUSHREAL8ARRAY(alf, r8*ncv*2/8)
  alf(:, :) = alf(:, :)*switch%prl_cur
!srv 16.06.08 }
!
  IF (switch%b2tqce_iout .NE. 0) THEN
...
!WG_TODO        close(70)
  ...
  IF (switch%b2npht_iout .NE. 0 .OR. switch%iout_b2wdat .EQ. 4) CALL &
&   MY_OUT_US(70, ncv, 0, tau, 'b2tqce_taue')
  CALL POPREAL8ARRAY(alf, r8*ncv*2/8)
  alfb(:, :) = switch%prl_cur*alfb(:, :)
  CALL POPREAL8ARRAY(sig, r8*ncv*2/8)
...
&       dv%ne(icv)
    ...
    END DO
  END IF
  CALL B2TLNL_B(ncv, switch, switch%icase_ee, pl%te, plb%te, pl%ti, plb%&
