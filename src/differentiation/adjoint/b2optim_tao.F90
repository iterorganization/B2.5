...
#include <petsc/finclude/petsctao.h>
      ...
      use b2mod_main_diff &
      , only : b2mn_init_b, b2mn_step, b2mn_step_b, b2mn_fin_b
      use b2mod_ad_diff &
      , only : nncf
      use b2us_data_diff
      implicit none
...
!  Allocate storage space for Hessian;
     ...
     , only : primal_iterations, gradient_iterations
      ...
      integer ipar, isigma, idum(0:2), imean, ishift
      character*3 str
      ...
      do ipar = 1, npar_opt - nsigma_opt - nmean_opt - nshift_opt
      ...
      do ipar = 1, nshift
        if (shiftopt(ipar)) then
          shift(ipar) = x_v(ishift) ! no rescale for shift for now
          write(str,"(I1)") ishift
...
! Experimental: write intermediate state file?
     ...
     , only : primal_iterations
      implicit none
      real(kind=r8) j(nncf)
      integer ipar, isigma, imean, ishift
      character*3 str
      PetscErrorCode ierr
      PetscInt dummy
      Vec XX
