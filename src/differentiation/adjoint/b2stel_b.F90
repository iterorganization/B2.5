...
!.specification
!
SUBROUTINE B2STEL_B(ncv, nfc, nvx, ns, ismain, switch, geo, geob, mpg, &
& pl, plb, dv, dvb, rt, rtb, rtw, rtwb, srw, srwb)
  USE B2MOD_TYPES
!      use b2mod_diag
  USE B2MOD_TALLIES_DIFF
  USE B2MOD_CONSTANTS
...
&             icv, is)*tempb2
          ...
          IF (branch .EQ. 0) THEN
            ...
            CALL POPREAL8ARRAY(t1, r8/8)
            temp0 = t0/(2.0_R8*2.0_R8)
            temp1 = pl%ua(icv, is) - pl%ua(icv, is-1)
            tempb2 = (1.0_R8-switch%boris)*mp*(am(is)+am(is-1))*t1b
            tempb1 = 2*temp1*pl%na(icv, is)*temp0*tempb2
            tempb = temp1**2*tempb2
            plb%na(icv, is) = plb%na(icv, is) + temp0*tempb
...
&             shi0_ionb(icv, 0)
            ...
            dvb%ni(icv, 0) = dvb%ni(icv, 0) + pl%ti(icv)*tempb2
            plb%ti(icv) = plb%ti(icv) + dv%ni(icv, 0)*tempb2
            plb%na(icv, is) = plb%na(icv, is) + t0*pl%tn(icv)*1.5_R8*&
&             t1ib - t0*pl%tn(icv)*1.5_R8*t1nb
...
&           icv)
        ...
        tempb1 = 2*temp1*pl%na(icv, is)*temp0*tempb
        tempb2 = temp1**2*tempb
        plb%na(icv, is) = plb%na(icv, is) + temp0*tempb2 + rf0*t0*&
&         sna0_ionb(icv, 0, is)
