...
!     for adjoint AD to avoid side-effect variables
  ...
  USE B2US_FEEDBACK_DIFF, ONLY : she_rad_tot
  IMPLICIT NONE
...
!   gradient     of useful results: she_rad_tot *(dv.ne) *(dv.ni)
!                *(dv.nn) *(rt.rlqa) *(rt.rlra) *(rt.rlsa) *(rt.rpi)
!                *(srw.she0) *(srw.shi0) *(srw.shn0) *(srw.smq0)
!                *(srw.sna0) *(srw.rqrad) *(srw.rqbrm) *(rtw.rsa)
!                *(rtw.rra) *(rtw.rqa) *(rtw.rrd) *(rtw.rbr) *(rtw.rqr)
...
!     for adjoint AD to avoid side-effect variables
  ...
  USE B2MOD_SUBSYS
  USE B2MOD_DIMENSIONS
  USE B2US_FEEDBACK_DIFF, ONLY : she_rad_tot, she_rad_totb
  IMPLICIT NONE
!   ..input arguments (unchanged on exit)
  INTEGER :: ncv, nfc, ns, ismain
  TYPE(SWITCHES), INTENT(IN) :: switch
...
!srv 30.06.08 11.09.09 }
  ...
  srwb%she0(:, 1) = srwb%she0(:, 1) + pl%te*wrk0b
  plb%te = plb%te + (srw%she0(:, 1)+dv%ne*srw%she0(:, 3))*wrk0b
  srwb%she0(:, 2) = srwb%she0(:, 2) + dv%ne*wrk0b
  dvb%ne = dvb%ne + (srw%she0(:, 2)+pl%te*srw%she0(:, 3))*wrk0b
  srwb%she0(:, 3) = srwb%she0(:, 3) + pl%te*dv%ne*wrk0b
  shn0_ionb = 0.D0
  shn0_recb = 0.D0
  shn0_ionb = srwb%shn0
  shn0_recb = srwb%shn0
  ...
  DO is=ns-1,0,-1
    DO icv=mpg%nci,1,-1
      ...
      IF (branch .NE. 0) THEN
        ...
        plb%na(icv, is) = plb%na(icv, is) + rtw%rbr(icv, is)*dv%ne(icv)*&
&         tempb2
...
&         tempb2
        rtwb%rrd(icv, is) = rtwb%rrd(icv, is) + dv%ne(icv)*tempb1
        ...
        dvb%ne(icv) = dvb%ne(icv) + rtw%rrd(icv, is)*tempb1 + pl%te(icv)&
&         *tempb2
