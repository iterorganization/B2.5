...
!    ..compute Bohm fchi, fche
        ...
        END DO
        fchi = qe*t0
        arg10 = -(qe*po(ix, -1)/te(ix, -1))
        result1 = EXPU2(arg10)
        fche = SQRT(1/(2*pi))*qe*ne(ix, -1)*vte*s1*result1
!sv 01.07.99
        fchy_dia = 0.0e0_R8
!sv 07.04.00
...
!    ..compute Bohm fchi, fche
        ...
        END DO
        fchi = qe*t0
        arg10 = -(qe*po(ix, ny)/te(ix, ny))
        result1 = EXPU2(arg10)
        fche = SQRT(1/(2*pi))*qe*ne(ix, ny)*vte*s1*result1
!sv
...
!  .. set the potential at inner lower plate for DND case with respect to outer plate
              arg10 = -(qe*(po(ix, iy)-po_in_lower)/te(ix, iy))
              result1 = EXPU2(arg10)
              fche = SQRT(1/(2*pi))*qe*ne(ix, iy)*vte*s1*result1
            ELSE
              arg10 = -(qe*po(ix, iy)/te(ix, iy))
              result1 = EXPU2(arg10)
              fche = SQRT(1/(2*pi))*qe*ne(ix, iy)*vte*s1*result1
            END IF
!    ..loop over species
...
!     ..compute csb
!sv 26.11.02
              csb = SQRT((rza(ix, iy, is)*te(ix, iy)+ti(ix, iy))/(am(is)&
&               *mp))
...
!  .. set the potential at inner upper plate for DND case with respect to outer plate
              arg10 = -(qe*(po(ix, iy)-po_in_upper)/te(ix, iy))
              result1 = EXPU2(arg10)
              fche = SQRT(1/(2*pi))*qe*ne(ix, iy)*vte*s1*result1
            ELSE
              arg10 = -(qe*po(ix, iy)/te(ix, iy))
