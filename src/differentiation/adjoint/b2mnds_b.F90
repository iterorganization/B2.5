...
!
!  Differentiation of b2mnds as a context to call adjoint code (with options context noISIZE r8):
...
!.specification
!
SUBROUTINE B2MNDS_B(ninp, nout, ncv, nfc, ns, nsb, ns0, switch)
  USE B2MOD_TYPES
  USE B2MOD_MATH_DIFF
  ...
  USE B2MOD_DIAG_DIFF, ONLY : label
...
!   ..read default physics common
  ...
  CALL B2RUCP_NODIFF(ninp(2), b2fparam_version, ns)
...
!   ..read atomic rate data
  ...
  CALL B2RURC_B(ninp(4), b2frates_version)
...
!   ..write physics parameters
  CALL B2WFCP_NODIFF(nout(0), 0, ns)
...
!   ..save physics parameters to nout(1)
  CALL B2WUCP_NODIFF(nout(1), newversion, ns)
...
!   ..write header to nout(4)
  ...
  CALL B2MWQ0_NODIFF(nout(4), ns, switch)
!
! ..return
  CALL SUBEND()
  RETURN
 100 CONTINUE
END SUBROUTINE B2MNDS_B
...
!.specification
!
SUBROUTINE B2MNDS_NODIFF(ninp, nout, ncv, nfc, ns, ns0, switch)
  USE B2MOD_TYPES
  USE B2MOD_MATH_DIFF
  USE B2MOD_VERSION_DIFF
  USE B2MOD_CONSTANTS
...
!   ..read atomic rate data
  ...
  adpak_used = INDEX(lblrc, 'ADPAK') .GT. 0
  CALL READ_B2MOD_PPOUT('rates', ninp(4), lblrc)
  CALL B2RURC_NODIFF(ninp(4), b2frates_version)
!   ..test rtns, rtzmin, rtzmax, rtzn
  CALL XERTST(ns .EQ. rtns, 'faulty input rtns--wrong rate table?')
  DO is=0,ns-1
    result1 = LTST(zamin(is), rtzmin(is))
