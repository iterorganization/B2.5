...
!     for adjoint AD to avoid side-effect variables
  USE B2MOD_AD_DIFF, ONLY : ncall_b2sifr, ncall_b2tlnl
  USE B2MOD_SUBSYS
  IMPLICIT NONE
!
...
!.specification
!
SUBROUTINE B2SIFR__B(ncv, nfc, nvx, ns, isb, switch, geo, geob, mpg, &
& mpgb, na, nab, ua, uab, te, teb, ti, tib, ni, ne, neb, ne2, ne2b, rza&
& , rzab, rz2, rz2b, ehxp, ehxpb, lnlam, lnlamb, cthe, cthi, st_ext, &
& smbch, smbchb, smbtf, smbtfb, smprb, smptb, smfrb, vti32, vti32b, gti&
& , gtib, gte, gteb)
  USE B2MOD_TYPES
!WG_RM      use b2mod_rates
!WG_RM      use b2mod_indirect
...
& ), neb(ncv), ne2b(ncv), rzab(ncv, 0:ns-1), rz2b(ncv, 0:ns-1), ehxpb(&
& ncv), lnlamb(ncv)
!   ..output arguments (unspecified on entry)
!srv 11.09.09
  REAL(kind=r8) :: smbch(ncv, 0:3), smbtf(ncv, 0:3), smprb(ncv), smptb(&
...
!   ..compute the Coulomb logarithm
  ...
  CALL B2TLNL_NODIFF(ncv, switch, icase, te, ti, ne, lnlam)
!   ..compute vti32
  CALL PUSHREAL8ARRAY(vti32, r8*ncv/8)
  vti32 = geo%cvvol/(ti/ev*SQRT(ti/ev))*geo%cvhz
!   ..compute gti,gte
...
&            , wrkvb, gti, gtib)
  CALL POPREAL8ARRAY(vti32, r8*ncv/8)
  temp = SQRT(ti/ev)
  ...
  CALL B2TLNL_B(ncv, switch, icase, te, teb, ti, tib, ne, neb, lnlam, &
&         lnlamb)
