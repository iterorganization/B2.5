...
& icsepimp, npfr_cvs, pfr_cvs
  USE B2MOD_EIRDIAG_DIFF
...
& she_eir_tot
  REAL(kind=r8), SAVE :: she_rad_totb, she_eir_totb
...
&   0.0_R8
  REAL(kind=r8), SAVE :: saved_fb_actuatorb(def_natm)=0.D0, &
& saved_fb_prevb(def_natm)=0.D0, fb_currentb(def_natm)=0.D0, fb_rescaleb&
& (def_natm)=0.D0, fb_current_prevb(def_natm)=0.D0
!
...
!                *(sr.sna) *(pl.na) *(pl.te) *(pl.ti) *(pl.tn)
...
!
            ...
            END DO
            CALL PUSHREAL8(fb_current(ifb), r8/8)
            fb_current(ifb) = fb_current(ifb)/SUM(geo%cvvol(pfr_cvs(1:&
&             npfr_cvs)))
!
...
!
            ...
            IF (switch%use_eirene .NE. 0) THEN
              CALL PUSHREAL8(fb_current(ifb), r8/8)
              fb_current(ifb) = fb_current(ifb) - (SUM(emolrad(:, :, 0))&
&               +SUM(eneutrad(:, :, 0))+SUM(eionrad(:, :, 0)))
...
&                       geometry_cdn) THEN
                    ...
                    ELSE
                      ...
                      GOTO 100
                    END IF
                    ad_from12 = mpg%fsfcp(ifs, 1)
                    DO k=ad_from12,mpg%fsfcp(ifs, 1)+mpg%fsfcp(ifs, 2)-1
                      CALL PUSHINTEGER4(ifc)
                      ifc = mpg%fsfc(k)
                      CALL PUSHINTEGER4(ic1)
                      ic1 = mpg%fccv(ifc, 1)
...
&                         ifc, 1))/SUM(geo%fchc(ifc, :))*geo%fcs(ifc)
                        IF (is .EQ. is_start + 1) THEN
            ...
            DO is=0,ns-1
              IF (b2espcr(is) .EQ. b2espcr(fb_species(ifb))) THEN
                ...
                DO icv=1,mpg%nci
                  IF (mpg%cvonclosedsurface(icv)) THEN
                    CALL PUSHCONTROL2B(1)
                  ELSE IF (mpg%cvreg(icv) .EQ. 0) THEN
                    CALL PUSHCONTROL2B(0)
                  ELSE
                    CALL PUSHREAL8(fb_current(ifb), r8/8)
              ...
              DO iatm=1,nnatmi
                IF (latmscl(iatm) .EQ. fb_species(ifb)) THEN
                  ...
                  DO icv=1,mpg%nci
                    ...
                    ELSE
                      CALL PUSHREAL8(fb_current(ifb), r8/8)
                      fb_current(ifb) = fb_current(ifb) + dab2(icv, iatm&
&                       , 1)*geo%cvvol(icv)
                      DO imol=1,nnmoli
                        IF (mlcmp(iatm, imol) .GT. 0) THEN
                          CALL PUSHREAL8(fb_current(ifb), r8/8)
                          fb_current(ifb) = fb_current(ifb) + dmb2(icv, &
&                           imol, 1)*mlcmp(iatm, imol)*geo%cvvol(icv)
                          CALL PUSHCONTROL1B(1)
                        ELSE
                          CALL PUSHCONTROL1B(0)
                        END IF
                ...
                END IF
              END DO
              CALL PUSHCONTROL5B(29)
            ELSE
              CALL PUSHCONTROL5B(30)
            END IF
          CASE DEFAULT
            CALL PUSHCONTROL5B(31)
          END SELECT
                    ...
                    DO icv=1,mpg%nci
                      ...
                      ELSE
                        ...
                        DO imol=1,nnmoli
                          IF (mlcmp(iatm, imol) .GT. 0) THEN
                            CALL PUSHCONTROL1B(1)
                            t0 = t0 + dmb2(icv, imol, 1)*mlcmp(iatm, &
&                             imol)*geo%cvvol(icv)
                          ELSE
                            CALL PUSHCONTROL1B(0)
                          END IF
...
&                 fb_rescaleb(ifb)/fb_current(ifb)**2
            ...
            ELSE
              ...
              temp1 = LOG(temp)/fb_beta(ifb)
              temp2 = LOG(fb_alpha(ifb))
              fb_currentb(ifb) = fb_currentb(ifb) - (1.0-TANH(temp1)**2)&
&               *temp2*2.0_R8**(temp2*(TANH(temp1)/temp0))*LOG(2.0_R8)*&
...
&                     fbreg(icv))*fb_currentb(ifb)
              ...
              ELSE IF (branch .EQ. 2) THEN
                ...
                CALL POPINTEGER4(is_start)
                CALL POPREAL8(fb_current(ifb), r8/8)
                tempb = fb_currentb(ifb)/(geo%cvvol(omp(icsepomp-1))+geo&
&                 %cvvol(omp(icsepomp)))
                fb_currentb(ifb) = 0.D0
                dvb%ne(omp(icsepomp-1)) = dvb%ne(omp(icsepomp-1)) + geo%&
&                 cvvol(omp(icsepomp-1))*tempb
                dvb%ne(omp(icsepomp)) = dvb%ne(omp(icsepomp)) + geo%&
&                 cvvol(omp(icsepomp))*tempb
              ELSE
                DO is=ns-1,0,-1
                  CALL POPCONTROL1B(branch)
                  IF (branch .NE. 0) THEN
...
&                         is) + geo%cvvol(icv)*charge_fracb(iss) + geo%&
&                         cvvol(icv)*sum_sb
                    END DO
                    CALL POPINTEGER4(icv)
                    CALL POPINTEGER4(ic2)
                      ...
                      DO ifc=ad_to6,ad_from6,-1
                        ...
                        IF (branch .NE. 0) THEN
                          IF (branch .NE. 1) THEN
                            ic1 = mpg%fccv(mpg%fsfc(ifc), 1)
                            ic2 = mpg%fccv(mpg%fsfc(ifc), 2)
                            tempb0 = geo%fcs(mpg%fsfc(ifc))*sum_eb
                            dvb%ne(ic1) = dvb%ne(ic1) + tempb0
                            dvb%ne(ic2) = dvb%ne(ic2) + tempb0
                          END IF
                          CALL POPREAL8(fb_current(ifb), r8/8)
                          tempb0 = geo%fcs(mpg%fsfc(ifc))*fb_currentb(&
&                           ifb)
                          plb%na(ic1, is) = plb%na(ic1, is) + tempb0
                          plb%na(ic2, is) = plb%na(ic2, is) + tempb0
                        END IF
                        CALL POPINTEGER4(ic2)
                        CALL POPINTEGER4(ic1)
                      END DO
                    ...
                    DO ifc=ad_to5,ad_from5,-1
                      ...
                      IF (branch .NE. 0) THEN
                        ...
                        END IF
                        CALL POPREAL8(fb_current(ifb), r8/8)
                        tempb0 = geo%fcs(mpg%fsfc(ifc))*fb_currentb(ifb)
                        plb%na(ic1, is) = plb%na(ic1, is) + tempb0
                        plb%na(ic2, is) = plb%na(ic2, is) + tempb0
                      END IF
                      CALL POPINTEGER4(ic2)
                ...
                IF (branch .NE. 0) THEN
                  IF (branch .NE. 1) THEN
                    ...
                    DO icv=ad_to7,ad_from7,-1
                      CALL POPCONTROL1B(branch)
                      IF (branch .NE. 0) dvb%ne(fbreg(icv)) = dvb%ne(&
&                         fbreg(icv)) + geo%cvvol(fbreg(icv))*sum_eb
                      CALL POPREAL8(fb_current(ifb), r8/8)
                      plb%na(fbreg(icv), is) = plb%na(fbreg(icv), is) + &
&                       geo%cvvol(fbreg(icv))*fb_currentb(ifb)
                    END DO
          ...
          ELSE IF (branch .LT. 12) THEN
            IF (branch .LT. 10) THEN
              IF (branch .EQ. 8) THEN
                ...
                CALL POPINTEGER4(is_start)
                CALL POPREAL8(fb_current(ifb), r8/8)
                dvb%ne(imp(icsepimp-1)) = dvb%ne(imp(icsepimp-1)) + &
&                 0.5_R8*fb_currentb(ifb)
...
&                 fb_currentb(ifb)
              ...
              ELSE
                ...
                DO ifc=ad_to8,ad_from8,-1
                  CALL POPREAL8(fb_current(ifb), r8/8)
                  dvb%fna(fbreg(ifc), 1, fb_species(ifb)) = dvb%fna(&
&                   fbreg(ifc), 1, fb_species(ifb)) + fb_currentb(ifb)
                END DO
                CALL POPINTEGER4(ifc)
                CALL POPREAL8(fb_current(ifb), r8/8)
                fb_currentb(ifb) = 0.D0
                ...
                IF (branch .EQ. 16) THEN
                  DO is=ns-1,0,-1
                    ...
                    IF (branch .NE. 0) THEN
                      IF (branch .NE. 1) THEN
                        CALL POPREAL8(fb_current(ifb), r8/8)
                        plb%na(omp(icsepomp-1), is) = plb%na(omp(&
&                         icsepomp-1), is) + 0.5_R8*fb_currentb(ifb)
                        plb%na(omp(icsepomp), is) = plb%na(omp(icsepomp)&
&                         , is) + 0.5_R8*fb_currentb(ifb)
                      END IF
                    END IF
                  END DO
                  CALL POPINTEGER4(is_end)
                ...
                ELSE
                  ...
                  fb_currentb(ifb) = 0.D0
                END IF
              ELSE IF (branch .NE. 18) THEN
                CALL POPREAL8(fb_current(ifb), r8/8)
                fb_currentb(ifb) = fb_currentb(ifb)/SUM(geo%cvvol(&
&                 pfr_cvs(1:npfr_cvs)))
                DO k=npfr_cvs,1,-1
                  icv = pfr_cvs(k)
                  CALL POPREAL8(fb_current(ifb), r8/8)
                  t0b = t0b + geo%cvvol(icv)*fb_currentb(ifb)
                  CALL POPCONTROL2B(branch)
                  IF (branch .EQ. 0) THEN
                    DO imol=nnmoli,1,-1
                      CALL POPCONTROL1B(branch)
                      IF (branch .NE. 0) CALL POPREAL8(t0, r8/8)
                    END DO
                    DO iatm=nnatmi,1,-1
                      CALL POPCONTROL1B(branch)
                      IF (branch .NE. 0) THEN
                  ...
                  ELSE
                    IF (branch .EQ. 1) THEN
                      CALL POPREAL8(t0, r8/8)
                      plb%na(icv, fb_species(ifb)) = plb%na(icv, &
&                       fb_species(ifb)) + pl%tn(icv)*t0b
                      plb%tn(icv) = plb%tn(icv) + pl%na(icv, fb_species(&
&                       ifb))*t0b
                    ELSE
                      CALL POPREAL8(t0, r8/8)
                      plb%na(icv, fb_species(ifb)) = plb%na(icv, &
&                       fb_species(ifb)) + pl%ti(icv)*t0b
                      plb%ti(icv) = plb%ti(icv) + pl%na(icv, fb_species(&
&                       ifb))*t0b
                    END IF
                    t0b = 0.D0
                  END IF
              ...
              she_eir_totb = she_eir_totb + fb_currentb(ifb)
              fb_currentb(ifb) = 0.D0
              DO is=ns-1,0,-1
                CALL POPCONTROL2B(branch)
              END DO
...
&                                              /8)
              CALL POPREAL8(fb_current(ifb), r8/8)
              srwb%rqbrm = srwb%rqbrm + fb_currentb(ifb)
              srwb%rqrad = srwb%rqrad + fb_currentb(ifb)
              fb_currentb(ifb) = 0.D0
              DO is=ns-1,0,-1
                CALL POPCONTROL2B(branch)
              END DO
                  ...
                  IF (branch .NE. 1) THEN
                    DO ic=2,1,-1
                      ...
                      IF (branch .NE. 0) THEN
                        ...
                        DO k=ad_to12,ad_from12,-1
                          CALL POPCONTROL2B(branch)
                          IF (branch .NE. 0) THEN
                            IF (branch .NE. 1) ifc = mpg%fsfc(k)
                            ic1 = mpg%fccv(ifc, 1)
                            ic2 = mpg%fccv(ifc, 2)
                            CALL POPREAL8(fb_current(ifb), r8/8)
                            tempb = geo%fcs(ifc)*fb_currentb(ifb)/SUM(&
&                             geo%fchc(ifc, :))
                            plb%na(ic1, is) = plb%na(ic1, is) + geo%fchc&
&                             (ifc, 2)*tempb
                            plb%na(ic2, is) = plb%na(ic2, is) + geo%fchc&
&                             (ifc, 1)*tempb
                          END IF
                          CALL POPINTEGER4(ic2)
                          CALL POPINTEGER4(ic1)
            ...
            ELSE
              ...
              IF (branch .EQ. 0) THEN
                ...
                fb_currentb(ifb) = fb_currentb(ifb) + abs0b
              ELSE
                CALL POPREAL8(abs0, r8/8)
                fb_currentb(ifb) = fb_currentb(ifb) - abs0b
              END IF
...
&                   **2)
                  ...
                  DO ifc=ad_to14,ad_from14,-1
                    CALL POPCONTROL1B(branch)
                    IF (branch .EQ. 0) THEN
                      CALL POPREAL8(fb_current(ifb), r8/8)
                      plb%te(icv) = plb%te(icv) + fb_currentb(ifb)/qe
                      fb_currentb(ifb) = 0.D0
                    ELSE
                      CALL POPREAL8(fb_current(ifb), r8/8)
                    END IF
...
&                                                 r8/8)
              ...
              ELSE IF (branch .EQ. 26) THEN
                ...
                DO ifc=ad_to15,ad_from15,-1
                  CALL POPCONTROL1B(branch)
                  IF (branch .EQ. 0) THEN
                    CALL POPREAL8(fb_current(ifb), r8/8)
                    dvb%ne(icv) = dvb%ne(icv) + fb_currentb(ifb)
                    fb_currentb(ifb) = 0.D0
                  ELSE
                    CALL POPREAL8(fb_current(ifb), r8/8)
                  END IF
                      ...
                      DO icv=mpg%nci,1,-1
                        ...
                        IF (branch .NE. 0) THEN
                          IF (branch .NE. 1) THEN
                            DO imol=nnmoli,1,-1
                              CALL POPCONTROL1B(branch)
                              IF (branch .NE. 0) CALL POPREAL8ARRAY(&
&                                                             fb_current&
&                                                             (ifb), r8/&
&                                                             8)
                            END DO
                ...
                IF (branch .NE. 0) THEN
                  DO icv=mpg%nci,1,-1
                    ...
                    IF (branch .NE. 0) THEN
                      IF (branch .NE. 1) THEN
                        CALL POPREAL8(fb_current(ifb), r8/8)
                        plb%na(icv, is) = plb%na(icv, is) + geo%cvvol(&
&                         icv)*fb_currentb(ifb)
                      END IF
                    END IF
                  END DO
...
&                              , rt%rza(1, is), rtb%rza(1, is), zafb)
 ...
 150      CALL POPINTEGER4(ad_from13)
          ...
          DO ifc=ad_to13,ad_from13,-1
            ...
            ic = mpg%fcbc(fbreg(ifc), 1)
            t1 = mpg%bcfcor(ic)
            t0b = t1*t0b/geo%fcs(fbreg(ifc))
            CALL POPREAL8(t0, r8/8)
            dvb%fht(fbreg(ifc), 0) = dvb%fht(fbreg(ifc), 0) + t0b
            dvb%fht(fbreg(ifc), 1) = dvb%fht(fbreg(ifc), 1) + t0b
            CALL POPREAL8(t1, r8/8)
            t0b = 0.D0
          END DO
          CALL POPINTEGER4(ifc)
