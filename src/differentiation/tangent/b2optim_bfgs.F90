...
program b2optim_bfgs
  ...
  ! tol_opt and maxiter come from b2mod_par_opt_diffv (same as TAO path)
  ...
  call bfgs_bound_solve(npar_opt, X, X_L, X_U,               &
                        ...
                        FormFunctionGradient,                &  ! expensive (b2mn_step_dv)
                        maxiter,                             &
...
contains
  ...
  !=====================================================================
  ! InitializeProblem
  ! Allocate and fill X, X_L, X_U from b2mod_par_opt_diffv arrays,
  ! mirroring what VecSetValue / VecAssemblyEnd did in b2optim_tao.
  !=====================================================================
  subroutine InitializeProblem(npar)
    use b2mod_par_opt_diffv, only : x0, xl, xu, par_rescale
  ...
  subroutine FormFunction(n, x_v, F)
    use b2mod_par_opt_diffv, only : sigma, mean, par_rescale
    use b2mod_ad_diffv,      only : primal_iterations, primal_res
    integer,          intent(in)  :: n
  ...
  ! accepted BFGS iterate.  Wraps b2mn_step_dv.
  ! Signature matches func_grad_if abstract interface.
  !=====================================================================
  subroutine FormFunctionGradient(n, x_v, F, g_v)
    use b2mod_diffsizes
    use b2us_io_diffv,       only : write_b2fstate
    use b2mod_version,       only : newversion, cfverw
    use b2mod_b2cmpa
    use b2mod_par_opt_diffv, only : par_rescale, sigma, mean
    use b2mod_ad_diffv,      only : primal_iterations, gradient_iterations, &
...
#ifdef TGT
    do ipar = 1, npar_opt
       g_v(ipar) = real(jdiff(ipar,1)*par_rescale(ipar), 8)
#endif
#ifdef ADJ
  ...
  subroutine reset_drifts_params(n, x_v)
    use b2mod_facdrift_exb_diffv, only : facdrift_scalar, fac_exb_scalar
    integer,          intent(in) :: n
    double precision, intent(in) :: x_v(n)
