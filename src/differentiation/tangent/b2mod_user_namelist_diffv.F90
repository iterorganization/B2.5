...
!
!
MODULE B2MOD_USER_NAMELIST_DIFFV
  USE B2MOD_TYPES
...
CONTAINS
!  Differentiation of alloc_b2mod_user as a context to call tangent code (with options multiDirectional context noISIZE r8):
...
!
!
  SUBROUTINE ALLOC_B2MOD_USER_DV(geo, geod, m, md0, ns, nlim, nmol, ntns&
&   , nbdirs)
  USE B2MOD_DIFFSIZES
    IMPLICIT NONE
!  Hint: nbdirsmax should be the maximum number of differentiation directions
...
! sc  Set prior range to (-inf,+inf)
      ...
      cf_regp = 0
      maptoomp = .false.
      CALL READ_B2MOD_USER_NAMELIST_DV(ns, ntns, geo, geod, m, md0, &
&                                nbdirs)
      user_initialised = .true.
!
      RETURN
    END IF
  END SUBROUTINE ALLOC_B2MOD_USER_DV
...
!  Differentiation of dealloc_b2mod_user as a context to call tangent code (with options multiDirectional context noISIZE r8):
!   Plus diff mem management of: b2voloncf:out b2data:out b2dataoncf:out
!
  SUBROUTINE DEALLOC_B2MOD_USER_DV(nbdirs)
!  Hint: nbdirsmax should be the maximum number of differentiation directions
  USE B2MOD_DIFFSIZES
    IMPLICIT NONE
    INTRINSIC ALLOCATED
    ...
    RETURN
  END SUBROUTINE DEALLOC_B2MOD_USER_DV
...
!  Differentiation of read_b2mod_user_namelist as a context to call tangent code (with options multiDirectional context noISIZE r
!8):
!   Plus diff mem management of: b2voloncf:in-out b2data:in-out
!                b2dataoncf:in-out m.cffcor:in-out geo.cvx:in geo.cvy:in
!                geo.vxx:in geo.vxy:in
!
  SUBROUTINE READ_B2MOD_USER_NAMELIST_DV(ns, ntns, geo, geod, m, md0, &
&   nbdirs)
...
! now allocate vector that will be used to interpolate solps onto data grid
    IF (ANY(cfread)) THEN
      ...
      ALLOCATE(b2voloncfd(nbdirsmax, ncf, ndata))
      DO nd=1,nbdirsmax
        b2voloncfd(nd, 1:ncf, 1:ndata) = 0.D0
      END DO
      ALLOCATE(b2voloncf(ncf, ndata))
!stores interpolated B2.5 volumes onto CF radial points
      ALLOCATE(b2dataoncfd(nbdirsmax, ndata))
      DO nd=1,nbdirsmax
        b2dataoncfd(nd, 1:ndata) = 0.D0
      END DO
...
!store here radial distance coordiante of SOLPS data
      ALLOCATE(b2datad(nbdirsmax, ndata))
      DO nd=1,nbdirsmax
        b2datad(nd, 1:ndata) = 0.D0
      END DO
...
!
    CALL SUBEND()
    RETURN
  END SUBROUTINE READ_B2MOD_USER_NAMELIST_DV
