...
!                b2dataoncf:in-out par_opt_phys:in-out fna_pinch:in-out
!                fhe_32:in-out fhi_vispar:in-out fna_nanom:in-out
!                fna_ch:in-out fhi_strange:in-out fhi_visq:in-out
!                fhi_anml:in-out fna_pll:in-out fhi_visper:in-out
!                fhe_thermj:in-out fhi_32:in-out fna_drift:in-out
!                fhe_strange:in-out fhi_inert:in-out fhe_cond:in-out
!                fhi_cond:in-out rtlsa:out rtlcx:out rtlqa:out
!                rtlra:out state_avg.na_mean:in-out state_avg.ua_mean:in-out
...
!                state.srw.sna0:in-out state.srw.b2stbc_sch:in-out
...
!                state.srw.rsana:in-out state.srw.rsahi:in-out
!                state.srw.rsamo:in-out state.srw.rrana:in-out
!                state.srw.rrahi:in-out state.srw.rramo:in-out
!                state.srw.rcxna:in-out state.srw.rcxhi:in-out
!                state.srw.rcxmo:in-out state.srw.rqahe:in-out
!                state.srw.rqrad:in-out state.srw.rqbrm:in-out
!                state.srw.b2sihs_joule:in-out state.srw.b2sihs_divue:in-out
!                state.srw.b2sihs_divua:in-out state.srw.b2sihs_exbe:in-out
!                state.srw.b2sihs_exba:in-out state.srw.b2sihs_visa:in-out
!                state.srw.b2sihs_fraa:in-out state.srw.b2sihs_str:in-out
!                state.srw.sna0_eir_tot:in-out state.srw.smo0_eir_tot:in-out
!                state.srw.sne0_eir_tot:in-out state.srw.she0_eir_tot:in-out
!                state.srw.shi0_eir_tot:in-out state.srw.shn0_eir_tot:in-out
!                state.srw.sch0_eir_tot:in-out state.rt.rlcx:in-out
...
!                sigma:in shift:in *par_opt_phys:in mean:in *fna_pinch:(loc)
!                *fhe_32:(loc) *fhi_vispar:(loc) *fna_nanom:(loc)
!                *fna_ch:(loc) *fhi_strange:(loc) *fhi_visq:(loc)
!                *fhi_anml:(loc) *fna_pll:(loc) *fhi_visper:(loc)
!                *fhe_thermj:(loc) *fhi_32:(loc) *fna_drift:(loc)
!                *fhe_strange:(loc) *fhi_inert:(loc) *fhe_cond:(loc)
!                *fhi_cond:(loc) int4l:(loc) int1l:(loc) int2l:(loc)
!                int3l:(loc) int0l:(loc) *rtlsa:in *rtlcx:in *rtlqa:in
!                *rtlra:in fb_target:(loc) fb_prev:(loc) fb_current:(loc)
...
!                geo.vxhz:(loc) geo.vxonedbsq:(loc) switch.keps_cd:in
...
!                state.srw.smo0:(loc) *(state.srw.smo0):(loc) state.srw.smq0:(loc)
!                *(state.srw.smq0):(loc) state.srw.sna0:(loc) *(state.srw.sna0):(loc)
!                state.srw.b2stbc_sch:(loc) state.srw.b2stbc_she:(loc)
!                state.srw.b2stbc_shi:(loc) state.srw.b2stbc_sne:(loc)
!                state.srw.b2stbc_shn:(loc) state.srw.b2stbc_skt:(loc)
!                state.srw.b2stbc_szt:(loc) state.srw.b2stbc_smo:(loc)
!                state.srw.b2stbc_sna:(loc) state.srw.b2stbm_sch:(loc)
!                state.srw.b2stbm_she:(loc) state.srw.b2stbm_shi:(loc)
!                state.srw.b2stbm_sne:(loc) state.srw.b2stbm_smo:(loc)
!                state.srw.b2stbm_sna:(loc) state.srw.b2stbr_sch:(loc)
!                state.srw.b2stbr_she:(loc) state.srw.b2stbr_shi:(loc)
!                state.srw.b2stbr_sne:(loc) state.srw.b2stbr_shn:(loc)
!                state.srw.b2stbr_skt:(loc) state.srw.b2stbr_szt:(loc)
!                state.srw.b2stbr_smo:(loc) state.srw.b2stbr_sna:(loc)
...
!                state.psnl.na:(loc) state.psnl.ua:(loc) state.psnl.po:(loc)
!                state.psnl.te:(loc) state.psnl.ti:(loc) state.psnl.tn:(loc)
...
!                state.psnl.fzt:(loc) state.psnl.kinrgy:(loc) state.psnc.na:(loc)
...
!                *(state.psnc.tn):(loc) state.psnc.kt:(loc) state.psnc.zt:(loc)
...
!                par_opt_phys:in fna_pinch:in fhe_32:in fhi_vispar:in
!                fna_nanom:in fna_ch:in fhi_strange:in fhi_visq:in
!                fhi_anml:in fna_pll:in fhi_visper:in fhe_thermj:in
!                fhi_32:in fna_drift:in fhe_strange:in fhi_inert:in
!                fhe_cond:in fhi_cond:in rtlsa:in rtlcx:in rtlqa:in
!                rtlra:in geo.cvbb:in geo.cvx:in geo.cvy:in geo.cvhz:in
...
!                state.psnl.ne:in state.psnl.ni:in state.psnl.fna:in
!                state.psnl.kinrgy:in state.psnc.na:in state.psnc.ua:in
...
!                state.psnc.kinrgy:in state.diag.aresco:in state.diag.aresmo:in
!                state.diag.acorpa:in state.diag.acorua:in state.diag.rescoreg:in
!                state.diag.resmoreg:in state.diag.reshereg:in
!                state.diag.reshireg:in
!
  SUBROUTINE B2MN_STEP_DV(switch, switchd, geo, geod, mpg, state, stated&
&   , state_ext, state_extd, state_avg, state_avgd, j, jd, nbdirs)
    USE B2MOD_TYPES
...
!
    ...
    CALL B2MNDR_1_DV(nout, ns, switch, switchd, geo, geod, mpg, state, &
...
!                par_opt_phys:out fna_pinch:out fhe_32:out fhi_vispar:out
...
!                fhe_cond:out fhi_cond:out rtlsa:out rtlcx:out
!                rtzmax:out rtlqa:out rtzn:out rtlra:out rtzmin:out
!                rpi0:out rz20:out rpt0:out rza0:out state_avg.na_mean:out
...
!                geo.cveb:out geo.fcbb:out geo.fcs:out geo.fchc:out
!                geo.fcht:out geo.fchz:out geo.fcvol:out geo.fcqgam:out
!                geo.fcqalf:out geo.fcqbet:out geo.fcpbs:out geo.fcpbshz:out
!                geo.fcbzb:out geo.fceb:out geo.vxbb:out geo.vxx:out
...
!                state_ext.sna:out state_ext.smo:out state.pl.na:out
...
!                state.co.cddi:out state.co.cvsahz_cl:out state.co.chcb:out
!                state.co.cvsa_an:out state.co.cvmahz:out state.co.cvsahz_eff:out
!                state.co.cthe:out state.co.cthi:out state.co.cvsa_cl:out
...
!                state.srw.b2stbr_sna:out state.srw.rsana:out state.srw.rsahi:out
!                state.srw.rsamo:out state.srw.rrana:out state.srw.rrahi:out
!                state.srw.rramo:out state.srw.rcxna:out state.srw.rcxhi:out
!                state.srw.rcxmo:out state.srw.rqahe:out state.srw.rqrad:out
!                state.srw.rqbrm:out state.srw.b2sihs_joule:out
!                state.srw.b2sihs_divue:out state.srw.b2sihs_divua:out
!                state.srw.b2sihs_exbe:out state.srw.b2sihs_exba:out
!                state.srw.b2sihs_visa:out state.srw.b2sihs_fraa:out
!                state.srw.b2sihs_str:out state.srw.sna0_eir_tot:out
!                state.srw.smo0_eir_tot:out state.srw.sne0_eir_tot:out
!                state.srw.she0_eir_tot:out state.srw.shi0_eir_tot:out
!                state.srw.shn0_eir_tot:out state.srw.sch0_eir_tot:out
!                state.rt.rlcx:out state.rt.rlqa:out state.rt.rlrd:out
!                state.rt.rlbr:out state.rt.rlra:out state.rt.rlsa:out
!                state.rt.rlza:out state.rt.rlz2:out state.rt.rlpt:out
!                state.rt.rlpi:out state.rt.rlqr:out state.rt.rza:out
!                state.rt.rz2:out state.rt.rpt:out state.rt.rpi:out
!                state.rtw.rsa:out state.rtw.rra:out state.rtw.rqa:out
!                state.rtw.rrd:out state.rtw.rbr:out state.rtw.rcx:out
!                state.rtw.rqr:out state.psnl.na:out state.psnl.ua:out
!                state.psnl.po:out state.psnl.te:out state.psnl.ti:out
!                state.psnl.tn:out state.psnl.kt:out state.psnl.zt:out
!                state.psnl.ne:out state.psnl.ni:out state.psnl.nn:out
!                state.psnl.fch:out state.psnl.fna:out state.psnl.fhi:out
!                state.psnl.fhe:out state.psnl.fhn:out state.psnl.fkt:out
!                state.psnl.fzt:out state.psnl.kinrgy:out state.psnc.na:out
!                state.psnc.ua:out state.psnc.po:out state.psnc.te:out
!                state.psnc.ti:out state.psnc.tn:out state.psnc.kt:out
!                state.psnc.zt:out state.psnc.ne:out state.psnc.ni:out
!                state.psnc.nn:out state.psnc.fch:out state.psnc.fna:out
!                state.psnc.fhi:out state.psnc.fhe:out state.psnc.fhn:out
!                state.psnc.fkt:out state.psnc.fzt:out state.psnc.kinrgy:out
!                state.diag.srcna:out state.diag.srcmo:out state.diag.srcpo:out
!                state.diag.srche:out state.diag.srchi:out state.diag.srcmt:out
!                state.diag.aresco:out state.diag.aresmo:out state.diag.acorpa:out
!                state.diag.acorua:out state.diag.rescoreg:out
