...
# To allow for statistical variance, we have a large tolerance for most of the variables (0.1).
# Except for the velocity, we check the maximum relative error for the basic quantities.
$MYPATH/b2diff.py --tolerance 0.1 --maxerr 'te ti na ni ne po' -i 'time|na|ua0|fne*|del*|res*|rrahi|rsahi|b2stbr_*|b2stbc_*|b2npmo_*|rcx*|smfr|csigin|dpa*|cdpa|fllim*' -v compare_results.log
