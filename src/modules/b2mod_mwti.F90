    ...
    if (ncall.eq.0) then
      ...
      if (.not.ex.or.stim.ge.0.0_R8) then
        ...
        call b2crtimecdf(filename, &
          nx, ny, nybl, nytl, nytr, nybr, nya, nyi, nc, ns, nbatch, write_2d, &
          ncid, .false., iret)
        ...
        iret = nf_inq_varid(ncid,'nastep',nastepid)
        if (nastepid.gt.0) then
...
#     include <netcdf.inc>
    ...
    if (.not.batch_only) then
      ...
      iret = nf_inq_dimid(ncid, 'nc', ncdim)
      call check_cdf_status(iret)
    end if
    ...
    if (.not.batch_only) then
      ...
      call check_cdf_status(iret)
      iret = nf_put_att_text(ncid, posepaid, 'long_name', 39, 'separatrix potential, outboard divertor')
      call check_cdf_status(iret)
      ...
      call check_cdf_status(iret)
      iret = nf_put_att_text(ncid, feesapid, 'long_name', 62, 'poloidal electron energy flux, into outboard separatrix throat')
      call check_cdf_status(iret)
      ...
      call check_cdf_status(iret)
      iret = nf_put_att_text(ncid, po3dlid, 'long_name', 27, 'potential, inboard divertor')
      call check_cdf_status(iret)
    ...
    call check_cdf_status(iret)
    iret = nf_put_att_text(ncid, posepa_stdid, 'long_name', 51, 'variance of separatrix potential, outboard divertor')
    call check_cdf_status(iret)
