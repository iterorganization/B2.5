    ...
    subroutine b2CreateMap( nx,ny,crx,cry,cflag,    &
        ...
        if (svc-svcDuplicates.eq.1) then
            call logmsg( LOGDEBUG, "b2CreateMap: found "    &
                &   //int2str(svc-svcDuplicates)//" special vertex (x-point),   &
                &   "//int2str(svc)//" including duplicates.")
        else
            call logmsg( LOGDEBUG, "b2CreateMap: found "    &
                &   //int2str(svc-svcDuplicates)//" special vertices (x-points),&
                &   "//int2str(svc)//" including duplicates.")
        end if
...
contains 
    ...
    !> b2 grid.
    !> The default is to check whether it isinside  the extended grid 
